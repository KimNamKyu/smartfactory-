...
namespace PosSystem
{
    public partial class CountView : Form
    {
            ...
            switch (btn.Name)
            {
                ...
                case "btn3":
                    ...
                    for (int i = 0; i < Orderlist.Count; i++)
                    {
                        ...
                        ht.Add("mNo", arr[0]);
                        ht.Add("oCount", arr[3]);
                        if (!api.Post("http://192.168.3.15:5000/sp_insert", ht))
                        {
                ...
                case "btn6":
                    ...
                    for (int i = 0; i < Orderlist.Count; i++)
                    {
                        ...
                        ht.Add("tNo", _tNo);
                        ht.Add("mNo", _mNo);
                        if (!api.Post("http://192.168.3.15:5000/sp_list_delete", ht))
                        {
                ...
                case "btn7": //전체 취소
                    for (int i = 0; i < Orderlist.Count; i++)
                    {
                        ...
                        ht.Add("tNo", _tNo);
                        if (!api.Post("http://192.168.3.15:5000/sp_delete", ht))
                        {
                ...
                case "btn8":
                    for (int i = 0; i < Orderlist.Count; i++)
                    {
                        ...
                        ht.Add("mNo", _mNo);
                        ht.Add("oCount", arr[3]);
                        if (!api.Post("http://192.168.3.15:5000/sp_update", ht))
                        {
                            MessageBox.Show("주문수정오류");
                            break;
                        }
        ...
        private void Category_Click(object sender, EventArgs e)
        {
            ...
            list = api.Select("http://192.168.3.15:5000/select", ht);
            if (list != null)
            {
